...
Python 的 `#` 注释、模块和函数文档字符串统一使用中文，`.gitignore` 中的说明注释也使用中文。函数名、变量名和类型注解继续使用英文，因为它们属于代码标识符，不是注释。

注释只解释代码本身无法清楚表达的原因，不重复描述代码正在做什么。第一阶段不增加英文注释自动检测脚本，以免扩大 MVP；每个里程碑通过文本搜索检查注释，并重新运行相关测试，确认文字调整没有改变程序行为。
